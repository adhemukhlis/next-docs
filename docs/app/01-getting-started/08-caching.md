...
{/_ xref to the sync io section - that section should also emphaize that these are not random or timestamp sync invokations, in the referenced section _/} `<Suspense>` provides a fallback UI while async work completes, but it does not itself opt a component into dynamic rendering. If a component only performs synchronous work, it will complete during prerendering regardless of whether it is wrapped in `<Suspense>`.
