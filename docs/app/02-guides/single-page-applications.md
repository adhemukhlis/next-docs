...
export type TodoAction = { type: 'add'; id: string; text: string } | { type: 'toggle'; id: string } | { type: 'edit'; id: string; text: string } | { type: 'delete'; id: string }
...
export async function todosReducer(todos: Todo[], action: TodoAction): Promise<Todo[]> {
			...
			<ul>
				{optimisticTodos.map((todo) => (
					<li key={todo.id}>
						...
						<button onClick={() => runAction({ type: 'delete', id: todo.id })}>Delete</button>
					</li>
				))}
			</ul>
