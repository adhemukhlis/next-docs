...
export function Board({ tasksPromise }) {
	const tasks = use(tasksPromise)
	const [optimisticTasks, moveTask] = useOptimistic(tasks, (currentTasks, action: { taskId: string; status: Status }) =>
...
export function DeleteButton({ deleteAction }: { deleteAction: () => void | Promise<void> }) {
	const [isPending, setIsPending] = useOptimistic(false)
...
<CommentCard
	comment={comment}
	deleteAction={comment.userName === 'You' ? deleteComment.bind(null, comment.id) : undefined}
/>
```
