...
> [!NOTE] This feature only applies to soft navigations into prefetched routes and Server Action calls from the current page. A full page reload while offline still fails because the browser needs the network to deliver the HTML; full offline loads would need a service worker (see the [Progressive Web Apps](/docs/app/guides/progressive-web-apps) guide).
...
export function ConnectivityFallback() {
	...
	return <p>{isOffline ? 'Waiting for connection to load this section...' : 'Loading...'}</p>
}
```
...
export function OfflineBanner() {
	...
	return <div role="status">Offline. Pending requests will retry once you are back online.</div>
}
```
...
export function PingForm() {
	...
	const label = pending ? (isOffline ? 'Pinging (offline, will retry)...' : 'Pinging...') : 'Ping'
