...
export function LocalDate({ date, options }: { date: string; options?: Intl.DateTimeFormatOptions }) {
	const id = useId()
	...
	return (
		<>
			...
			<InlineScript html={`{var n=document.getElementById("${id}");if(n)n.textContent=new Date("${date}").toLocaleDateString(undefined,${JSON.stringify(options)})}`} />
		</>
	)
}
```
