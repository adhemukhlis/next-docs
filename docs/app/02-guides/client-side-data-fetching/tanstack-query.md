...
import { getCurrentUserId, markActivityRead as markActivityReadInDatabase } from './data'
import { activityCache } from './activity-cache'
...
import { cacheLife, cacheTag } from 'next/cache'
import { defaultShouldDehydrateQuery, QueryClient, type DehydratedState, type QueryKey } from '@tanstack/react-query'
...
export async function dehydrate(queries: HydratedQuery[], options: HydrationOptions): Promise<DehydratedState> {
...
async function ProductData({ id }: { id: string }) {
	...
	const state = await dehydrate([{ queryKey: productCache.key(id), data: product }], { tags: [productCache.tag(id)] })
