...
export function ProductAutocomplete({ query }: { query: string }) {
	const { data = [], error, isLoading } = useSWR(query ? `/api/products?query=${encodeURIComponent(query)}` : null, fetcher)
...
function ProductResults({ query }: { query: string }) {
	const { data } = useSWR(`/api/products?query=${encodeURIComponent(query)}`, fetcher, { suspense: true })
