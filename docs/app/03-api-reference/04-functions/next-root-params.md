...
export async function generateStaticParams() {
	...
	const posts = await fetch(`https://api.example.com/posts?lang=${language}`).then((res) => res.json())
...
export default async function DocsLayout(props: LayoutProps<'/docs/[...path]'>) {
